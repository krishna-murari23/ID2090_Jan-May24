...
#Generating Random points in vicinity of current node of tree
    def rand_point(self):
        # Probability of choosing goal point
        probab_goal = 0.1  # This value we can adjust , based of how greedy we can be 
        if np.random.uniform() < probab_goal:
             return self.goal.point
        else:
            # Sample random points biased towards the goal
            # Weighted average of current point and goal point (L2 NORM)
            rand_x = np.random.uniform(self.bounds[0], self.bounds[0] + self.bounds[2])
            rand_y = np.random.uniform(self.bounds[1], self.bounds[1] + self.bounds[3])
            greedy_point = np.array(self.goal.point) - np.array([rand_x, rand_y])
            # Scaling the distance to control the bias parameter
            greedy_pt = np.array([rand_x, rand_y]) + 0.1 * greedy_pt
            return greedy_pt.tolist()
