...
#Backtrack the path from the goal node to the start node in the tree
    def backtrack_path(self):
        ...
        return path[::-1]
#The class and function declaration remains the same until this point , after which we declare a function to smoothen the path given
# by RRT. For this specific case I used Cubic Spline Interpolation.
    def smooth_path(self, path):
        ...
        new_t = np.linspace(0, 1, len(path) * 10)
        smooth_path = np.column_stack((cs_x(new_t), cs_y(new_t)))
        
        return smooth_path
#Here , we plot the smoothened function in a similar fashion to normal RRT 
