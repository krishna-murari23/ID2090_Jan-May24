...
if ! python3 -c "import sympy, yaml" &> /dev/null; then
    echo "Error: Required Python packages 'sympy' and 'PyYAML' are not installed."
    exit 1
fi
...
if [ ! -f "$1" ]; then
    echo "Error: YAML file '$1' not found."
    exit 1
fi

# Run the Python script with the provided YAML file
python3 - <<END
import yaml
...
def check_unitary(matrix):
    return matrix * matrix.adjoint() == eye(matrix.shape[0]) #Unitary Matrices are those that satisfy A*Complex Conj(A) = I
...
# Put the data in the YAML file from obj.yml using the function yaml.load()
...
# Convert the YAML data to SymPy matrix or DomainMatrix
...
#print(eigenvecsmatrix) (matrix with the eigenvectors)
